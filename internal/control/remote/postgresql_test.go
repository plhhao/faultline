...
func TestPostgreSQLManagedAPI(t *testing.T) {
	source := strings.NewReplacer("protocol: http1", "protocol: postgresql", "http://127.0.0.1:19090", "postgresql://127.0.0.1:5432", "action: respond, phase: before_upstream_request, status: 503", "action: delay, phase: after_commit, duration: 100ms").Replace(fixture)
	s := setupConfig(t, source)
	cookie, csrf := login(t, s)
	w := call(s, "GET", "/api/capabilities", nil, cookie, csrf)
	if w.Code != 200 || !strings.Contains(w.Body.String(), `"postgresql":["delay","hold_response","close_connection"]`) || !strings.Contains(w.Body.String(), `"postgresql":["after_commit"]`) {
